...
- **User settings:** wake/sleep, weekends, Google Calendar flags, `allowSplitScheduling` / `minSplitMinutes`, IANA **`timeZone`** (filled once on first login, editable in Settings; source of truth for the scheduling engine), **`hiddenGoogleCalendarIds`** (which Google calendars the Calendar page hides)  
