...
import { getPhaseSchedulingTimeBounds } from './phaseSchedulingBounds';
import {
...
export const TaskSchedulingStep: React.FC<Props> = ({
  ...
  const phaseId = useWatch({ control, name: 'phaseId' }) as string | undefined;
  ...
  const fieldRenderers: Record<SchedulingFieldKey, React.ReactNode> = {
    ...
    preferredStartTime: (
      <PreferredStartField control={control} errors={errors} phaseTimeBounds={phaseTimeBounds} />
    ),
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {resolvedEventType === 'daily_routine' && phaseTimeBounds && !phaseTimeBounds.overnight ? (
        <p className="text-sm text-ide-muted">
          In preferred start, times outside the selected phase (
          {`${phaseTimeBounds.startHHMM}–${phaseTimeBounds.endHHMM}`}) are disabled.
        </p>
      ) : resolvedEventType === 'daily_routine' && phaseTimeBounds?.overnight ? (
        <p className="text-sm text-ide-muted">
          This phase crosses midnight; all preferred start times stay selectable.
        </p>
      ) : null}
