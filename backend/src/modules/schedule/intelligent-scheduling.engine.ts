...
const MIN_PLACE_MINUTES = 5;

/** Minutes still to place after fully ended auto slots (non-recurring). */
export function remainingFlexibleMinutes(
  estimatedTimeInMinutes: number,
  endedAutoMinutes: number,
): number {
  return Math.max(0, estimatedTimeInMinutes - Math.max(0, endedAutoMinutes));
}

/** True when a From/Until or due window already ended — do not place or error. */
export function isFlexibleDeadlinePast(
  deadline: Date | string | null | undefined,
  nowMs: number,
): boolean {
  if (!deadline) return false;
  const t =
    deadline instanceof Date ? deadline.getTime() : Date.parse(String(deadline));
...
export class IntelligentSchedulingEngine {
  ...
  async captureTaskScheduleSnapshot(
    userId: string,
  ): Promise<TaskScheduleSnapshot[]> {
    const tasks = await this.taskRepo.find({
      ...
      select: [
        ...
        'scheduledEndTime',
        'googleEventId',
        'googleEventCalendarId',
      ],
    });
    return tasks.map((t) => ({
      id: t.id,
      scheduledStartTime: t.scheduledStartTime
        ? new Date(t.scheduledStartTime).toISOString()
        : null,
      scheduledEndTime: t.scheduledEndTime
        ? new Date(t.scheduledEndTime).toISOString()
        : null,
      googleEventId: t.googleEventId ?? null,
      googleEventCalendarId: t.googleEventCalendarId ?? null,
    }));
  }
  ...
  async run(
    ...
    const unscheduledIdSet = new Set(unscheduledIds);
    if (persist && unscheduledIds.length) {
      await this.scheduledRepo
        .createQueryBuilder()
        .delete()
        .from(ScheduledTask)
        .where('taskId IN (:...ids)', { ids: unscheduledIds })
    ...
    const endedMinutesByTask = new Map<string, number>();
    if (movableIds.length) {
      const keptAuto = persist
        ? await this.scheduledRepo.find({
            where: { taskId: In(movableIds), isAutoGenerated: true },
          })
        : autoBefore.filter(
            (row) =>
              movableIdSet.has(row.taskId) &&
              row.scheduledEndTime.getTime() <= nowMs,
          );
      for (const row of keptAuto) {
        const mins =
          (row.scheduledEndTime.getTime() - row.scheduledStartTime.getTime()) /
          60000;
        if (mins > 0) {
          endedMinutesByTask.set(
            row.taskId,
            (endedMinutesByTask.get(row.taskId) ?? 0) + mins,
          );
        }
      }
    }
    ...
    if (!persist) {
      const afterByTask = this.projectAutoAfter(
        autoBefore,
        movableIdSet,
        unscheduledIdSet,
        nowMs,
    ...
    for (const task of movable) {
      ...
      const live = await this.taskRepo.findOne({ where: { id: task.id } });
      if (!live || live.status !== TaskStatus.TODO) {
  ...
  private attemptPlaceTask(
    ...
    const rules = getEventTypeRules(task.eventType);
    if (isFlexibleDeadlinePast(task.deadline, nowMs)) {
      return {
        ok: true,
        segments: [],
        beyondHorizon: false,
    ...
    const durationMin = remainingFlexibleMinutes(
      task.estimatedTimeInMinutes,
      endedMinutesByTask.get(task.id) ?? 0,
    );
    if (durationMin < MIN_PLACE_MINUTES) {
      return {
        ok: true,
        segments: [],
        beyondHorizon: false,
    ...
    const minChunk =
      settings.allowSplitScheduling && rules.splittable && task.allowSplit
        ? Math.min(durationMin, Math.max(5, settings.minSplitMinutes ?? 30))
        : durationMin;
    const maxChunk =
      settings.allowSplitScheduling && rules.splittable && task.allowSplit
        ? Math.min(durationMin, Math.max(minChunk, settings.maxSplitMinutes ?? minChunk))
        : durationMin;
  ...
  private projectAutoAfter(
    autoBefore: ScheduledTask[],
    ...
    for (const row of autoBefore) {
      const open = row.scheduledEndTime.getTime() > nowMs;
      const dropped =
        open &&
        (movableIds.has(row.taskId) || unscheduledIds.has(row.taskId));
      if (!dropped) push(row);
    }

    for (const [taskId, segs] of newSegments) {
      for (const seg of segs) {
        push({
          taskId,
          scheduledStartTime: seg.start,
          scheduledEndTime: seg.end,
          isAutoGenerated: true,
        } as ScheduledTask);
      }
    }

    for (const list of afterByTask.values()) {
      list.sort(
        (a, b) =>
          a.scheduledStartTime.getTime() - b.scheduledStartTime.getTime(),
      );
