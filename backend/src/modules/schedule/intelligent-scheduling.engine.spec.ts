...
describe('remainingFlexibleMinutes', () => {
  it('subtracts fully ended auto minutes and never goes negative', () => {
    expect(remainingFlexibleMinutes(120, 60)).toBe(60);
    expect(remainingFlexibleMinutes(60, 60)).toBe(0);
    expect(remainingFlexibleMinutes(30, 90)).toBe(0);
  });
});

describe('isFlexibleDeadlinePast', () => {
  it('is true when the Until/deadline instant is already over', () => {
    const now = Date.parse('2026-09-11T00:31:00+03:00');
...
describe('IntelligentSchedulingEngine', () => {
  ...
  beforeEach(() => {
    ...
    taskRepo = {
      ...
      findOne: jest.fn(async ({ where }: { where: { id: string } }) => {
        const rows = (await taskRepo.find()) as Task[];
  ...
  it('keeps completed auto segments as busy and only deletes still-open ones', async () => {
    ...
    taskRepo.find.mockResolvedValue([existing, incoming]);
    scheduledRepo.find.mockResolvedValue([
      {
        id: 'st-done',
        taskId: existing.id,
    ...
    expect(scheduledRepo.deleteQb?.andWhere).toHaveBeenCalledWith(
      'scheduledEndTime > :now',
      expect.objectContaining({ now: expect.any(Date) }),
    );

    const byTask = extractTaskSegmentsByTaskId(scheduledRepo.save.mock.calls);
    expect(byTask.get('done-slot')).toBeUndefined();
    expect(byTask.get('incoming')?.[0]).toEqual([
      atLocalTimeIso(1, 9),
      atLocalTimeIso(1, 10),
    ]);
  });

  it('places only remaining minutes when part of a split already ended', async () => {
    jest.setSystemTime(new Date('2026-04-20T15:00:00.000Z'));
    const existing = makeTask({
      id: 'partial',
      name: 'Long task',
    ...
    taskRepo.find.mockResolvedValue([existing]);
    scheduledRepo.find.mockResolvedValue([
      {
        id: 'st-done',
        taskId: existing.id,
    ...
    const byTask = extractTaskSegmentsByTaskId(scheduledRepo.save.mock.calls);
    expect(byTask.get('partial')?.[0]).toEqual([
      atLocalTimeIso(1, 9),
      atLocalTimeIso(1, 10),
    ]);
  });

  it('skips a TODO whose From/Until window already ended instead of erroring', async () => {
    jest.setSystemTime(new Date('2026-04-21T00:31:00.000Z'));
    taskRepo.find.mockResolvedValue([
      makeTask({
        id: 'barber',
        name: 'call barber',
    ...
    expect(result.errors).toHaveLength(0);
    expect(extractTaskSegmentsByTaskId(scheduledRepo.save.mock.calls).get('barber')).toBeUndefined();
  });

  it('does not write a slot if the task was completed during replan', async () => {
    const inbox = makeTask({
      id: 'done-during-replan',
      name: 'Call barber',
      estimatedTimeInMinutes: 30,
      phases: [phaseWorkday],
    });
    taskRepo.find.mockResolvedValue([inbox]);
